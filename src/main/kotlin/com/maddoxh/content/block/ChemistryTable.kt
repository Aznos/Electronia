...
class ChemistryTable(settings: Settings): MachineBlock(settings) {
    ...
    companion object {
        val CODEC: MapCodec<ChemistryTable> = createCodec(::ChemistryTable)
    }

    override fun createBlockEntity(pos: BlockPos, state: BlockState): BlockEntity {
        return ChemistryTableBlockEntity(pos, state)
    }
    ...
    private fun updateBE(world: World, pos: BlockPos, state: BlockState, player: PlayerEntity): Boolean {
        if(!world.isClient) {
            val be = world.getBlockEntity(pos) as? ChemistryTableBlockEntity ?: return false
    ...
    override fun <T : BlockEntity> getTicker(
        ...
        return BlockEntityTicker<T> {
                ...
                blockEntity: T ->
            (blockEntity as ChemistryTableBlockEntity).serverTick()
        }
    }
